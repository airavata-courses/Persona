...
public class FileServiceImpl implements FileService {
	...
	public void deleteFileById(Long id) throws Exception {
		Document dbDocument = documentRepo.findById(id)
				.orElseThrow(() -> new ResourceNotFoundException("File not found id :" + id));
	...
	public void shareDocuments(ShareDocumentRequest shareDocumentRequest) {
		...
		User userToShare = userRepo.findByUserName(shareDocumentRequest.getUsernameToShare())
				.orElseThrow(() -> new ResourceNotFoundException(
						"User not found username :" + shareDocumentRequest.getUsernameToShare()));
		...
		try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
			try (ZipOutputStream zos = new ZipOutputStream(baos)) {
				...
				for (Long id : ids) {
					...
					String[] originalNameSplit = curFileName.split("\\.(?=[^\\.]+$)");
					String originalName = originalNameSplit[0];
					String extension = originalNameSplit[1];
					for (int i = 1; !distinctFileNames.add(curFileName); i++) {
						curFileName = originalName + "_" + i + "." + extension;
					}
	...
	public Set<DocumentResponse> getFilesByUsername(String userName) {
		...
		Set<DocumentResponse> documentResponses = user.getDocuments().stream()
				.map(DocumentMapper::documentToDocumentResponse).collect(Collectors.toSet());

		documentResponses.forEach(
				documentResponse -> documentResponse.setData(googleDrive.getByteArray(documentResponse.getFileId())));
